...
# * Overlapping research areas: **Corpus linguistics**, **Natural language processing**, **Distance reading**
# * Additional skills: **Regular Expressions**, some **Shell commands**
...
# <markdowncell>
# So, let's jump right into Python with a simple bit of code.
# Below is a cell with some pre-filled code, and some comments. Highlight it and hit shift+enter to run the code.

# <codecell>
# A simply welcome message printer.
...
# <markdowncell>
# Sometimes, however, this might not work out like we expect. Nope that we get some brackets and so on when we try to print the items in a list as a string.
...
# <markdowncell>
# NLTK is a Python Library for working with written language data. It is free and extensively documented. Many areas we'll be covering are treated in more detail in the NLTK Book, available for free in online from [here](http://www.nltk.org/book/).
...
# <markdowncell>
# 'Concordance' shows you a word in context. 'Similar' will find words used
# in similar contexts; remember it is not looking for synonyms, 
...
# Earlier, we created a simple function that welcomed you and told you the time.

# In NLTK, we can create a function called **lexical diversity** that runs a single line of code. We can then call this function to quickly determine the lexical density of a corpus or subcorpus.
...
# The speeches were transcribed years ago. Optimal Character Recognition (OCR) was used to digitise the transcripts. This means that the texts are not of perfect quality.
